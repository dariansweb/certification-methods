...
export default function Footer() {
  return (
    <footer className="bg-slate-500 text-center text-xs text-zinc-500 py-6 border-t border-zinc-200 mt-0">
      <div className="mt-6">
        <h3 className="text-zinc-50 font-semibold mb-3 text-sm">Reading Lists</h3>
        <div className="flex flex-wrap justify-center gap-3">
          {readingListDomains.map((domain) => (
            <Link
              key={domain.slug}
            ...
            >
              ...
              <div className="mt-1 text-[13px] font-medium leading-tight">{domain.name}</div>
            </Link>
          ))}
        </div>
      </div>
      <p className="text-slate-50">&copy; {new Date().getFullYear()} Codex of Mastery — Study Well, Study Deep.</p>
